...
[LEARN:workflow] Manuscript PDF snapshots: before replacing `manuscript/main.pdf` with a new Quarto build, copy it to `manuscript/archive/main_YYYY-MM-DD_HHMM.pdf`. Repeat on a regular cadence (e.g., end of substantive editing sessions or weekly). See `manuscript/archive/README.md`.

[LEARN:workflow] Report circulation (2026-04-06): The manuscript PDF (`manuscript/main.pdf` or `main-draft.pdf`) was sent to **Eric Grodsky** for substantive feedback and to **Tommy Jaine** for LFS. Await feedback before the next revision cycle.

[LEARN:workflow] Report circulation (2026-07-09): Report sent to **Erin G. at the Wisconsin State Journal** for a story she is working on — findings may appear in press. The version sent reflects the corrected school-dedup numbers and the 16-school reporting threshold (MMSD White-Black r = 0.23). If asked about earlier figures citing r = 0.18, that was the pre-QC value from the name-splitting bug. Project paused at user request after this send.
...
[LEARN:data] CROSS-STATE EXTENSION (2026-07-08): 8 states with scripted school-by-race downloads: CA (CAASPP research files — use `_csv_` caret-delimited variant, NOT `_ascii_` fixed-width; group IDs 74=Black/78=Hispanic/80=White), TX (TAPR CGI POST; STAAR1 set = race×grades 3-8; column code C{grp}{gg}A{subj}{lvl}{yy}{N/D/R}, lvl 12=Meets; form params changed at 2024 redesign), IL (Report Card xlsx `IAR` sheet; pooled by-race rates 2024+ only; no per-race N), NY (SRC Access DBs, need brew mdbtools; per-grade ELA3-8 rows — the ELA3_8 combined rows are All-Students-only; exclude statewide code 111111111111), OH (report-card blob store, needs SAS token from SPA bundle; BUILDING_ETHNIC files; pct only, no N), GA (GOSA EOG CSVs; Proficient+Distinguished; ALL GRADES pooled rows), NC (accrpt.tops.ncsu.edu Disag zips; pct_glp; drop LEA/SEA/SB aggregate school_codes), NJ (NJSLA per-subject-grade xlsx; header row 3; race label "African American"; L4+L5). FL (Akamai-blocked), MI (Power BI only), MN (timeouts), MA (ASP.NET form) require manual download — see DATA.md.

[LEARN:analysis] CROSS-STATE FINDING (2026-07-08, revised 07-09): statewide school-level White-Black ELA correlations are 0.58-0.76 in all 8 comparison states (WI = 0.46 is the lowest); MMSD's r≈0.23 is far below every reportable place. REPORTING THRESHOLD: per user decision, correlations from subsamples with <16 schools are NOT reported anywhere (MIN_N_CORR=16 in script 15) — this removes every college-town analog except Athens-Clarke County GA (0.33, 18 schools; vs statewide 0.59). The college-town interpretation of the MMSD pattern is a hypothesis resting on that one district; Evanston/Ithaca/Berkeley/Chapel Hill point estimates must not be cited. White-Hispanic correlations are strong (0.6-0.8) in every reportable sample including MMSD (0.63). Report: manuscript/cross-state.qmd → cross-state.pdf; scripts 13/14/15 in analysis/.

[LEARN:analysis] SCHOOL DEDUP BUG FIXED (2026-07-08 QC): scripts 09/10/15 originally pooled schools grouping by NAME columns as well as codes; schools renamed across years (e.g. MMSD Falk→Milele Chikasa Anana, Glendale→Dr Virginia Henderson) were split into two scatter points, and NY schools with missing district names were silently dropped by pivot_table. After fixing (group by codes/IDs only, attach latest name), MMSD White-Black r moved 0.18→0.23 (n 41→38); statewide/urban correlations changed by <0.02. Always group panel data by codes, never names.
