...
quietly infix                  ///
  ...
  int     mortucod    172-174  ///
  byte    mortucodld  175-176  ///
  double  mortwt      177-184  ///
  double  mortwtsa    185-192  ///
...
label var mortucod   `"Underlying cause of death (ICD-10)"'
...
label define mortucod_lbl 001 `"Salmonella infections"'
label define mortucod_lbl 002 `"Shigellosis and amebiasis"', add
label define mortucod_lbl 003 `"Other intestinal infections"', add
label define mortucod_lbl 005 `"Respiratory tuberculosis"', add
label define mortucod_lbl 006 `"Other tuberculosis"', add
label define mortucod_lbl 007 `"Whooping cough"', add
label define mortucod_lbl 008 `"Scarlet fever and erysipelas"', add
label define mortucod_lbl 009 `"Meningococcal infection"', add
label define mortucod_lbl 010 `"Septicemia"', add
label define mortucod_lbl 011 `"Syphilis"', add
label define mortucod_lbl 012 `"Acute poliomyelitis"', add
label define mortucod_lbl 013 `"Arthropod-borne encephalitis"', add
label define mortucod_lbl 014 `"Measles"', add
label define mortucod_lbl 015 `"Viral hepatitis"', add
label define mortucod_lbl 016 `"Human immunodeficiency virus"', add
label define mortucod_lbl 017 `"Malaria"', add
label define mortucod_lbl 018 `"Other infectious parasitic disease"', add
label define mortucod_lbl 020 `"Cancer of lip, oral cavity, pharynx"', add
label define mortucod_lbl 021 `"Cancer of esophagus"', add
label define mortucod_lbl 022 `"Cancer of stomach"', add
label define mortucod_lbl 023 `"Cancer of colon, rectum, anus"', add
label define mortucod_lbl 024 `"Cancer of liver and bile ducts"', add
label define mortucod_lbl 025 `"Cancer of pancreas"', add
label define mortucod_lbl 026 `"Cancer of larynx"', add
label define mortucod_lbl 027 `"Cancer of trachea, bronchus, lung"', add
label define mortucod_lbl 028 `"Malignant melanoma of skin"', add
label define mortucod_lbl 029 `"Cancer of breast"', add
label define mortucod_lbl 030 `"Cancer of cervix uteri"', add
label define mortucod_lbl 031 `"Cancer of uterus"', add
label define mortucod_lbl 032 `"Cancer of ovary"', add
label define mortucod_lbl 033 `"Cancer of prostate"', add
label define mortucod_lbl 034 `"Cancer of kidney and renal pelvis"', add
label define mortucod_lbl 035 `"Cancer of bladder"', add
label define mortucod_lbl 036 `"Cancer of brain, nervous system"', add
label define mortucod_lbl 038 `"Hodgkin's disease"', add
label define mortucod_lbl 039 `"Non-Hodgkin's lymphoma"', add
label define mortucod_lbl 040 `"Leukemia"', add
label define mortucod_lbl 041 `"Multiple myeloma"', add
label define mortucod_lbl 042 `"Other lymph and blood cancer"', add
label define mortucod_lbl 043 `"Other, unspecified neoplasms"', add
label define mortucod_lbl 044 `"In situ and benign neoplasms"', add
label define mortucod_lbl 045 `"Anemias"', add
label define mortucod_lbl 046 `"Diabetes mellitus"', add
label define mortucod_lbl 048 `"Malnutrition"', add
label define mortucod_lbl 049 `"Other nutritional deficiencies"', add
label define mortucod_lbl 050 `"Meningitis"', add
label define mortucod_lbl 051 `"Parkinson's disease"', add
label define mortucod_lbl 052 `"Alzheimer's disease"', add
label define mortucod_lbl 055 `"Rheumatic fever/heart disease"', add
label define mortucod_lbl 056 `"Hypertensive heart disease"', add
label define mortucod_lbl 057 `"Hypertensive heart, renal disease"', add
label define mortucod_lbl 059 `"Acute myocardial infarction"', add
label define mortucod_lbl 060 `"Other acute ischemic heart disease"', add
label define mortucod_lbl 062 `"Atherosclerotic cardiovascular disease"', add
label define mortucod_lbl 063 `"Other chronic ischemic heart disease"', add
label define mortucod_lbl 065 `"Acute and subacute endocarditis"', add
label define mortucod_lbl 066 `"Pericardium disease, myocarditis"', add
label define mortucod_lbl 067 `"Heart failure"', add
label define mortucod_lbl 068 `"All other forms of heart disease"', add
label define mortucod_lbl 069 `"Primary hypertension, renal disease"', add
label define mortucod_lbl 070 `"Cerebrovascular diseases"', add
label define mortucod_lbl 071 `"Atherosclerosis"', add
label define mortucod_lbl 073 `"Aortic aneurysm and dissection"', add
label define mortucod_lbl 074 `"Other diseases of arteries, capillaries"', add
label define mortucod_lbl 075 `"Other disorders of circulatory system"', add
label define mortucod_lbl 077 `"Influenza"', add
label define mortucod_lbl 078 `"Pneumonia"', add
label define mortucod_lbl 080 `"Acute bronchitis and bronchiolitis"', add
label define mortucod_lbl 081 `"Acute lower resp. infection, unspec"', add
label define mortucod_lbl 083 `"Bronchitis, chronic and unspecified"', add
label define mortucod_lbl 084 `"Emphysema"', add
label define mortucod_lbl 085 `"Asthma"', add
label define mortucod_lbl 086 `"Other chronic lower resp. disease"', add
label define mortucod_lbl 087 `"Pneumoconioses"', add
label define mortucod_lbl 088 `"Pneumonitis from solids, liquids"', add
label define mortucod_lbl 089 `"Other respiratory system diseases"', add
label define mortucod_lbl 090 `"Peptic ulcer"', add
label define mortucod_lbl 091 `"Diseases of appendix"', add
label define mortucod_lbl 092 `"Hernia"', add
label define mortucod_lbl 094 `"Alcoholic liver disease"', add
label define mortucod_lbl 095 `"Other chronic liver disease"', add
label define mortucod_lbl 096 `"Cholelithiasis, gallbladder disease"', add
label define mortucod_lbl 098 `"Acute nephrotic syndrome"', add
label define mortucod_lbl 099 `"Chronic glomerulonephritis"', add
label define mortucod_lbl 100 `"Renal failure"', add
label define mortucod_lbl 101 `"Other disorders of kidney"', add
label define mortucod_lbl 102 `"Infections of kidney"', add
label define mortucod_lbl 103 `"Hyperplasia of prostate"', add
label define mortucod_lbl 104 `"Female pelvic inflammatory disease"', add
label define mortucod_lbl 106 `"Pregnancy with abortive outcome"', add
label define mortucod_lbl 107 `"Other pregnancy, birth complications"', add
label define mortucod_lbl 108 `"Certain perinatal conditions"', add
label define mortucod_lbl 109 `"Congenital mal or deformations"', add
label define mortucod_lbl 110 `"Abnormal clinical, lab findings, nec"', add
label define mortucod_lbl 111 `"All other diseases (Residual)"', add
label define mortucod_lbl 114 `"Motor vehicle accidents"', add
label define mortucod_lbl 115 `"Other land transport accidents"', add
label define mortucod_lbl 116 `"Water, air, space transport accidents"', add
label define mortucod_lbl 118 `"Falls"', add
label define mortucod_lbl 119 `"Accidental discharge of firearms"', add
label define mortucod_lbl 120 `"Accidental drowning"', add
label define mortucod_lbl 121 `"Accidental fire, smoke exposure"', add
label define mortucod_lbl 122 `"Accidental poisoning"', add
label define mortucod_lbl 123 `"Other nontransport accidents"', add
label define mortucod_lbl 125 `"Suicide by discharge of firearms"', add
label define mortucod_lbl 126 `"Suicide by other or means"', add
label define mortucod_lbl 128 `"Homicide by firearm discharge"', add
label define mortucod_lbl 129 `"Homicide by other means"', add
label define mortucod_lbl 130 `"Legal intervention"', add
label define mortucod_lbl 132 `"Firearm discharge, unknown intent"', add
label define mortucod_lbl 133 `"Other, unspecified events"', add
label define mortucod_lbl 134 `"War operations, their sequelae"', add
label define mortucod_lbl 135 `"Medical, surgical complications"', add
label define mortucod_lbl 999 `"NIU"', add
label values mortucod mortucod_lbl
