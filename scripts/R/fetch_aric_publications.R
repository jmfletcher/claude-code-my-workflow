# ============================================================
# Parse ARIC publications from website scrape + PubMed JSON
# Source: datasets/ARIC/raw/publications.json
# ============================================================
...
dataset_name <- if (length(args) >= 1) args[1] else "ARIC"
...
if (!file.exists(json_path)) {
  stop(
    "Missing ", json_path,
    ". Run: .venv-ffcws/bin/python scripts/aric/ingest_publications.py"
  )
}
...
if (nrow(parsed_tbl) == 0) {
  stop("No ARIC publications parsed")
}
...
out_path <- file.path(dirs$raw, "aric_publications.csv")
...
cat("PubMed-enriched author lists:", pubmed_n, "\n")
