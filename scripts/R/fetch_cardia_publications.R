# ============================================================
# Parse CARDIA publications from Zenodo community JSON
# Source: datasets/CARDIA/raw/publications.json
# ============================================================
...
dataset_name <- if (length(args) >= 1) args[1] else "CARDIA"
...
if (!file.exists(json_path)) {
  stop(
    "Missing ", json_path,
    ". Run: .venv-ffcws/bin/python scripts/cardia/ingest_publications.py"
  )
}
...
parse_record <- function(rec) {
  ...
  tibble(
    ...
    zenodo_id = rec$zenodo_id %||% NA_character_,
    title = str_squish(title),
    pub_year = as.integer(rec$pub_year %||% NA_integer_),
    doi = rec$doi %||% NA_character_,
    resource_type = rec$resource_type %||% NA_character_,
...
if (nrow(parsed_tbl) == 0) {
  stop("No CARDIA publications parsed")
}
...
out_path <- file.path(dirs$raw, "cardia_publications.csv")
