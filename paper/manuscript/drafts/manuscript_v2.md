...
Recent estimates of US children with a deceased parent are anchored to demographic-rate models in which decedents are assumed to have the same dependent-child count as same-cell survivors. The assumption is treated as a sensitivity, not a measurement. Using the NHIS Linked Mortality File 1986-2018 we test it directly. Within the demographic cells used by Schlüter et al. (2024) for drug-overdose and firearm parental deaths 1999-2020, the cumulative child count is **691,000** under our preferred specification -- **42 % below the published 1.19 million**. The correction is robust across four NHIS calibration paths (all-cause, intent-stratified, two cause-specific specifications) and survives a back-of-envelope augmentation for non-resident fathers. For the Villaveces et al. (2025) all-cause 2021 prevalent estimate, the pooled US correction is small (-3 %) but race-stratified corrections range from **-20 % for non-Hispanic Asian / Pacific Islander children to +26 % for non-Hispanic American Indian or Alaska Native children**. We document that the calibration effect is structurally driven by *household composition* on the day of death -- single-adult parents face 1.3-1.9× the mortality of coupled parents in every race × sex cell -- not by within-cell biological-fertility heterogeneity. The published estimates therefore answer a custodial-orphanhood question with biological-orphanhood inputs. We argue the field should report two numbers per paper.

---

## 1. Introduction

The US bereavement-demography literature has grown rapidly since the COVID-19 episode. Villaveces et al. (2025) report 2.91 million US children with a deceased parent or caregiver-grandparent in 2021. Schlüter et al. (2024) report 1.19 million cumulative children of drug-overdose or firearm parental decedents over 1999-2020. Potter et al. (2025), Verdery et al. (2024), Hillis et al. (2021, 2022), and the Annual Review essay of Smith-Greenaway, Verdery, & Carr (2025) round out a field that has gone from a handful of papers to a body of work in five years.

These estimates share a modeling architecture and an assumption. The architecture is the matrix-kinship recurrence of Caswell (2019, 2020) and Caswell & Song (2021), implemented for applied work in the `DemoKin` R package (Williams, Sánchez Pérez, & Alburez-Gutiérrez, 2023). The assumption is that within demographic cells defined by parent age, sex, race / ethnicity, year, and sometimes geography, **adults who die during the modeling horizon have the same dependent-child count as adults who survive.** This assumption is natural when the modeler does not have data on the actual children of the actual decedents. It is also a load-bearing assumption: if decedents have systematically fewer (or more) dependent children than same-cell survivors, the demographic-rate orphanhood total is biased correspondingly.
...
This paper measures the within-cell decedent-vs-survivor ratio directly using the NHIS Linked Mortality File (NHIS-LMF), an NCHS data product that links NHIS respondents from sample years 1986-2018 to the National Death Index. For each cell $c = (\text{sex}, \text{race / eth}, \text{age band}, \text{decade})$ we observe the weighted-mean count of co-resident minor children among adults who *died* during NDI follow-up and among adults who *survived*. Their ratio, call it $\kappa_c$, is the calibration parameter the published literature fixes at 1.0 and then perturbs in robustness checks.

Three findings, in order of substantive importance:

1. **The published Schlüter cumulative is approximately 40 % too high.** Cumulative US children of drug-overdose and firearm parental decedents 1999-2020 are 691,000 under our preferred specification using the detailed `MORTUCOD` underlying-cause recode; 716,000 using the coarser `MORTUCODLD` leading-cause field; 809,000 using all-cause $\kappa$. The published Schlüter total is 1.19 million.

2. **The all-cause 2021 prevalence is approximately right at the national level but biased in opposite directions by race / ethnicity.** $\Delta \%$ vs the equal-fertility baseline ranges from **-20 % for NH Asian / PI children, -14 % for NH White children, and -6 % for NH Black children**, to **-1 % for Hispanic children and +26 % for NH AIAN children**. The pooled "All" correction is -3 %, masking ~50 percentage points of cell-level dispersion.

3. **The calibration parameter measures household-composition selection on mortality, not biological-fertility heterogeneity.** Single-adult parents face 1.3-1.9× the mortality of coupled parents in every race × sex cell in NHIS-LMF, and have lower $K_{\text{alive}}$ at the survey address. Decedents are over-represented in single-parent households, and that drives $\kappa < 1$ in most cells. The published estimates measure biological orphanhood; our $\kappa$-calibrated estimates measure custodial orphanhood. We argue these are different policy concepts and the field should report both.

The remainder of the paper proceeds as follows. Section 2 reviews the published literature and locates our contribution. Section 3 describes the data. Section 4 lays out the matrix-kinship engine, the $\kappa$ estimator, and the cause-specific extension; technical details are deferred to a Supplementary Methods Appendix. Section 5 reports the all-cause and cause-specific results with bootstrap confidence intervals. Section 6 discusses two implications -- that $\kappa$ is measuring household structure, and that custodial and biological orphanhood are different quantities -- and explains why this matters for policy. Section 7 concludes with a research agenda anchored on a state-level vital-records linkage pilot.
...
We grouped the major US orphanhood papers (Fletcher 2026a) by whether their core method allows within-cell fertility heterogeneity (Table N1).
...
| Stover et al. (Spectrum, 2014, 2024) | UNAIDS HIV/AIDS | Yes (HIV stage, ART status) |

Pattern: exceptions appear when (a) the cause-of-death has a documented epidemiological link to fertility (HIV, cervical cancer), or (b) a risk-proximate survey approximates decedent characteristics (NSDUH past-year drug users). The exceptions do not appear for the US all-cause case or for non-overdose causes. Until now there has been nothing analogous to UNAIDS Spectrum for the US literature.

### 2.3 What the published sensitivity analyses do not address

Both Villaveces and Schlüter perturb their fertility input parametrically and report that "robust" describes the substantive conclusion. Neither analysis assesses:

- Whether the parametric form is right. If the true $\kappa$ varies by race / ethnicity and the parametric sensitivity is applied uniformly, the analysis is reassuring about pooled headlines but uninformative about race-stratified ones.
...
The NHIS-LMF is built by NCHS by probabilistic linkage of National Health Interview Survey respondents to the National Death Index. The 2019 public-use release covers NHIS sample years 1986-2018 with mortality follow-up through December 31, 2019; an updated 2022 release was published in January 2026 extending follow-up through end-2022. We use the 2019 file as the primary sample and treat refresh to the 2022 file as a planned extension in §7.
...
Sample sizes: 1.25 million person-records across 1986-2018 with eligible linkage and either `mortstat == 1` (died) or `mortstat == 2` (alive at follow-up cutoff). Restricting to adult parents (age ≥ 18 with at least one co-resident minor) yields **193,245 parent-respondents, of whom 7,437 died during follow-up.** Cause-specific subsamples are smaller: 697 drug-overdose decedents and 1,566 firearm decedents under the `MORTUCOD` definitions described in §3.5.
...
For the parental + grandparent-caregiver combined total comparable to Villaveces's 2.91 M headline, we add a flow-stock accounting layer using ACS Table S1002 ("grandparents responsible for grandchildren") for 2010, 2015, 2019, and 2021. Adult mortality among ages 50-79 is from CDC WONDER. The grandparent layer is *not* NHIS-calibrated because NHIS has no grandchild head-count variable for adults outside the family unit.
...
We use the standard time-varying two-sex matrix-kinship recurrence (Caswell & Song, 2021) for the parental kin block, implemented as a Python port of the `DemoKin` R package and validated against the Villaveces et al. (2025) all-cause parental-only baseline to within 1 % for 2021. Implementation details (block-matrix specification, initial parent-age distribution, absorbing vs incident dead-block coding) are in Supplementary Methods A.
...
where $\kappa_s(a-x, t-x)$ is the calibration ratio at parent-age-at-focal-birth $(a-x)$ in cohort year $(t-x)$ for sex $s$. Interpretation: among adults in cell $c$ who die, the expected co-resident-minor count at the moment of death is $\kappa_c$ times the cell-average; the implied orphanhood mass in that cell scales by $\kappa_c$ accordingly.
...
4. **Cause-specific ($\kappa$ from `MORTUCOD`), BROAD (preferred)**: NHIS code 122 + 126 + 129 mapped to NCHS X40-X44, X60-X64, X85, Y10-Y14 (drug) and NHIS codes 119+125+128+132 mapped to NCHS W32-W34, X72-X74, X93-X95, Y22-Y24 (firearm).

The BROAD specification matches the Schlüter (2024) target denominator and is therefore the closest apples-to-apples comparison.

### 4.5 Decomposition of the Schlüter gap

The published 1.19 M minus our 691 K = 499 K gap can be decomposed:

- **(i) Pipeline differences** (different male-fertility assumptions, age-band aggregation, denominator coverage): our pipeline run with $\kappa = 1$ at *both* NHIS K-alive and the published Schlüter male-fertility assumption.
...
- **(iii) Definitional gap** (custodial vs biological orphanhood): the magnitude of the non-resident-father augmentation discussed in §6.2.
...
Our Python re-implementation of the time-varying two-sex matrix-kinship recurrence produces a 2021 US prevalent parental-orphanhood count of 2,240,912 children under 18. Villaveces et al. (2025) report 2,910,000 combining parents and caregiver-grandparents in 2021; the 670 K gap is the grandparent-caregiver layer, which our independent flow-stock accounting layer recovers to 2,711,000 combined (within 7 % of Villaveces). The 2020 → 2021 jump in our model is +156 K (+7 %), aligned with Villaveces in direction and magnitude.

### 5.2 All-cause $\kappa$ calibration

Table 1 reports 2021 race-stratified prevalent orphanhood under baseline (equal-fertility) and NHIS-calibrated specifications. Figure 2 reports the time-varying $\Delta \%$ by race / ethnicity over 2000-2021.
...
*Notes:* Unit of observation: US child age 0-17. Baseline column: matrix-kinship engine with $\kappa = 1$ assumption (equal-fertility). Calibrated column: same engine with $\kappa$ replaced by NHIS-LMF cell-level estimates per §4.3. CIs from 200 PSU-clustered bootstrap replicates on the NHIS calibration component.

The pooled "All" correction is -3.4 % and statistically indistinguishable from zero. The race-stratified pattern is large and signed-opposite. NH White (-14 %) and NH Asian / PI (-20 %) are statistically distinguishable from zero on the lower bound. NH AIAN (+26 %) is point-significant but with a wide CI given NHIS-LMF small-sample issues for that group. NH Black and Hispanic are essentially flat at the point estimates but with wide CIs.

**Figure 2.** Race-stratified NHIS-$\kappa$ correction to US prevalent parental orphanhood, 2000-2021.

![Figure 2: race-stratified Δ % over 2000-2021](figures/figure2_race_stratified.png)

The time-trajectory in Figure 2 reveals dynamics that are invisible in a single-year snapshot. NH AIAN $\Delta \%$ grows from +5 % in 2000 to +27 % in 2021, tracking the documented rise of deaths of despair in young AIAN cohorts. NH Black $\Delta \%$ flips from -13 % in 2000 to near 0 % in 2017 -- consistent with closing of the NH Black mortality gap among parents of school-age children over the decade. NH White $\Delta \%$ is stably around -22 % from 2000-2014, narrowing to -14 % by 2021 as overdose mortality concentrates in younger NH White parents. NH Asian / PI $\Delta \%$ deepens steadily from 0 % to -20 %. Hispanic $\Delta \%$ shrinks from +13 % to roughly 0 %. The published equal-fertility model treats all five trajectories as flat at zero.

### 5.3 Cause-specific recalibration of Schlüter (2024)

Figure 1 plots the annual cumulative trajectory under each calibration specification.

**Figure 1.** Cumulative US children of drug-overdose and firearm parental decedents, 1999-2020, by calibration specification.

![Figure 1: cumulative Schlüter trajectory by specification](figures/figure1_schluter_cumulative.png)

All four NHIS-calibrated trajectories track each other closely from 1999-2010, then diverge from the naive (kids per living adult) trajectory as overdose mortality concentrates in younger parental cohorts. By 2020 the spread among NHIS specifications is 691 K (MORTUCOD BROAD) to 809 K (all-cause $\kappa$). The Schlüter published total at 2020 is 1.19 M.
...
Why is $\kappa < 1$ in most cells? Table 3 documents the mechanism using the household-structure stratification of §4.6.
...
*Notes:* Mortality rates pooled across NHIS sample years 1986-2018, MEPS-weighted. Coupled = married/cohabiting respondent with exactly 2 adults in `fmx`. Sole adult = exactly 1 adult in `fmx`.

In every cell, single-adult parents face 1.3-1.9× the mortality of coupled parents. They also have lower mean dependent-child count at the survey address ($\bar{n}_k$ for sole-adult NH White women = 1.67 vs 1.97 for coupled). The combination produces $\bar{n}_k^{\text{died}} < \bar{n}_k^{\text{alive}}$ in the aggregate -- not because decedents have fewer biological children, but because decedents are over-represented in the household structure that has lower co-resident-minor counts.

This is methodologically important. Within household structure (coupled or sole-adult), $\kappa$ is much closer to 1.0: $\kappa_{\text{coupled, NH White, F}} = 0.90$; $\kappa_{\text{sole-adult, NH White, F}} = 0.92$. The aggregate $\kappa < 1$ is composition-driven. The published demographic-rate model is missing not biological fertility heterogeneity but household-structure selection.
...
The mechanism in §5.4 is general. Wherever single-parent households have elevated mortality relative to coupled-parent households -- a pattern documented in every NHIS race × sex cell, in CDC linked mortality more broadly, in CPS-NDI linkages, and in international comparators -- decedents will have systematically lower co-resident-minor counts than survivors of the same age, sex, and race / ethnicity. The published demographic-rate orphanhood headlines therefore answer a question they do not realize they are answering: "if a given parent had the cell-average household composition, how many co-resident minors would they have lost?" The actual question of policy interest -- "how many co-resident minors does the population of decedents actually have?" -- requires the structural information that NHIS-LMF provides and that natality-data-only pipelines do not.

This is a general methodological lesson. Anywhere a demographic-rate model uses cell-average fertility schedules, the same composition selection on mortality can bias the result. Maternal cancer (Guida et al., 2022). HIV / AIDS bereavement (Stover et al.). Conflict bereavement (Alburez-Gutierrez et al., 2024). The published estimates in each of these literatures may be biased by the same household-structure composition effect that we document for the US case; the magnitude is empirically open.

### 6.2 Custodial vs biological orphanhood is a definitional choice
...
A back-of-envelope augmentation using ACS-based non-resident-father rates (30 % for NH White, 55 % for NH Black, 35 % for Hispanic, 15 % for NH Asian / PI, 50 % for NH AIAN) and a one-minor-child-each assumption raises $\bar{n}_{k,\text{father}}^{\text{died}}$ by 15-30 % (Appendix A.4). For the all-cause Villaveces 2021 target, this places the data-implied count between **2.17 M (custodial, NHIS-calibrated) and 2.4-2.5 M (biological, NHIS + non-resident-father)**. The Villaveces published parental-only number (2.91 M combined minus our 0.55 M grandparent layer = 2.36 M parental-only) lands inside this range, consistent with the biological interpretation.

For the Schlüter 1999-2020 cumulative, the same augmentation places the biological-orphanhood number around 800-900 K -- still well below the published 1.19 M.
...
The implications for US policy depend on the operational definition.

**SSI Survivor Benefits** require the surviving child to demonstrate financial dependency on the deceased parent. The relevant concept is closer to custodial orphanhood. NHIS-calibrated number: **2.17 M in 2021 all-cause** + 0.55 M grandparent layer = 2.71 M combined.
...
**Epidemiological surveillance of the deaths-of-despair episode** -- the use case where the Schlüter 1.19 M figure has been most prominently cited (Case & Deaton, 2015, 2020) -- the relevant target is debatable. For sizing surviving-parent support services: biological (~800-900 K cumulative drug + firearm 1999-2020). For assessing actual disruption to children's day-to-day caregiving: custodial (691 K cumulative). Both are 30-45 % below the published 1.19 M.
...
The reconciliation is that the published equal-fertility model overstates custodial orphanhood by ~8 % (the $\kappa$ correction) but understates the *biological* concept it claims to measure by ~5-10 % (because it does not properly account for non-resident parental status either). The two errors approximately cancel at the pooled US level. They do *not* cancel at the cause-specific level (Schlüter is 40 % too high under any specification we consider) and they do *not* cancel at the race-stratified level (the spread is 50 percentage points).

The fact that the headline national number is approximately right by accident does not reduce the importance of getting the cell-level structure right. Race-stratified resource allocation, cause-specific surveillance, and the headline drug-and-firearm cumulative are all systematically miscalibrated.
...
2. **$\kappa$ is estimated at decade × age band × race / ethnicity × sex resolution.** Annual variation within a decade is absorbed into the decade-level estimate. Figure 2 partially mitigates this concern by showing the cohort-induced annual variation that survives the decade-level smoothing.
3. **Cause-specific $\kappa$ is estimated on NHIS sample years 1986-2004 only**, because the detailed `MORTUCOD` field is not published for later years. We apply the resulting $\kappa$ to NCHS 1999-2020 deaths under a constant-effects-over-time assumption. We test stability by re-running the cause-specific calibration with the coarser `MORTUCODLD` field (available all years); the headline shifts by <5 %.
...
5. **Bootstrap CIs cover NHIS sampling component only.** They do not include sampling error in NCHS denominators or CDC WONDER population estimates, nor model uncertainty in the kinship recurrence. A total-error CI would be wider.
...
We recommend that future bereavement-demography papers report two headlines -- custodial and biological -- and label them. The two concepts answer different policy questions and produce different numbers. A field that reports a single number ambiguously is a field that lets the headline do work the definition cannot bear.
...
*Notes:* Identical-K is a selection result, not a biological-fertility result. We are conditioning on fathers who had minor children at the survey address.
...
**Table A3** (= Table 3 in main text). Pooled NHIS-LMF mortality rate by sex × race / ethnicity × household structure.
