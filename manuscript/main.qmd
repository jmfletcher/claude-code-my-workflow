...
```{=latex}
\vspace{0.4em}
{\footnotesize\textit{Note:} All gaps are in ACT scale-score points (White minus Black). Total
Gap ($T$) $=$ Between ($B$) + Within ($W$) by construction. ``Between'' is the share of the
gap attributable to Black and White students attending districts with different average ACT
scores; ``Within'' is the share present within the same district. $N$ = number of districts
with non-suppressed data for both racial groups in that year. COVID year 2020--21 excluded;
2019--20 had no state testing.}
```
...
#| tbl-cap: "WKCE-Era Mean Reading Scale Scores: MMSD and Peer Districts (Black and Hispanic, 2009--14)"
#| output: asis
...
```{python}
#| label: tbl-wkce-gr10
#| tbl-cap: "WKCE Grade-10 State-Level Scale-Score Gaps (Reading and Mathematics)"
#| output: asis
...
```{python}
#| label: tbl-crossera
#| tbl-cap: "Cross-Era Summary: Key Racial Achievement Gap Indicators (WKCE vs.\ Forward Exam)"
#| output: asis
...
cross_df = pd.DataFrame(rows, columns=[
    "Era", "Subject", "Gap", "Magnitude", r"\\% Between", r"\\% Within", "MMSD vs.\ Peers"
])
pub_table(
    cross_df,
