...
def pub_table(df, colalign=None, floatfmt=".1f", note=None, small=False, resizebox=False):
    """Print a publication-quality booktabs LaTeX table (use with #| output: asis).

    Parameters
    ----------
    df        : DataFrame with columns already renamed for display.
    colalign  : tuple of 'left'/'right'/'center' per column; defaults to
                left for first column, right for remainder.
    floatfmt  : tabulate float format string (default '.1f').
    note      : Optional note string (plain text; wrapped in footnotesize minipage).
    small     : If True, wraps table in footnotesize environment.
    resizebox : If True, wraps table in resizebox to fit text width.
                Do not combine with small=True.
    """
    if colalign is None:
    ...
    if note:
        note_block = (
            ...
            + r"\vspace{4pt}\raggedright\footnotesize\textit{Note:} " + note + "\n"
            + r"\end{minipage}"
        )
        latex = latex + "\n" + note_block
    print(latex)
...
#| tbl-cap: "Statewide Racial Proficiency Gaps by Year and Subject (Grade-Enrollment-Weighted, Primary Years)"
#| tbl-pos: H
#| output: asis
...
pivot.columns = ["B--W ELA", "B--W Math", "H--W ELA", "H--W Math"]
pivot = pivot.round(1).reset_index()
pivot = pivot.rename(columns={"year": "School Year"})
...
pub_table(
    ...
    floatfmt=".1f",
    note=(
        r"Gaps = White proficiency rate minus minority proficiency rate, in percentage points (pp). "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"Values are grade-enrollment-weighted averages across grades 3--8. "
        r"Primary years exclude 2019--20 (no testing) and 2020--21 (COVID disruption). "
        r"ELA = English Language Arts."
    ),
)
```
...
```{python}
#| label: tbl-act-gaps
#| tbl-cap: "Statewide ACT Score Gaps by Year (Grade 11, Enrollment-Weighted)"
#| tbl-pos: H
#| output: asis

# Pivot to wide: rows = year, columns = gap by subject (BW and HW, key subjects)
_keep_subj = ["Composite", "Mathematics", "English"]
_g = act_gaps[act_gaps["subject"].isin(_keep_subj)].copy()

_bw = (
    _g[_g["gap_pair"] == "BW"]
    .pivot(index="year", columns="subject", values="gap")
    .rename(columns={s: f"B--W {s}" for s in _keep_subj})
)
_hw = (
    _g[_g["gap_pair"] == "HW"]
    .pivot(index="year", columns="subject", values="gap")
    .rename(columns={s: f"H--W {s}" for s in _keep_subj})
)
act_tbl_wide = (
    _bw.join(_hw)
    .round(2)
   ...
   "B--W Composite", "B--W Mathematics", "B--W English",
   "H--W Composite", "H--W Mathematics", "H--W English"]]

pub_table(
    act_tbl_wide,
    colalign=("left",) + ("right",) * 6,
    floatfmt=".2f",
    resizebox=True,
    note=(
        r"Gaps = White mean ACT score minus minority mean ACT score (ACT scale: 1--36). "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"Values are district-enrollment-weighted state averages. Grade 11 only. "
        r"COVID year 2020--21 excluded; 2019--20 had no state testing."
    ),
)
```
...
#| tbl-cap: "Black--White ACT Composite Score Gap: Within-Between District Decomposition (Grade 11)"
#| tbl-pos: H
#| output: asis

_d = (
    act_decomp[
        (act_decomp["gap_pair"] == "BW") & (act_decomp["subject"] == "Composite")
    ][["year", "T", "B", "W", "B_pct", "W_pct", "n_units_both"]]
    .assign(
        T      = lambda x: x["T"].round(2),
        B      = lambda x: x["B"].round(2),
        W      = lambda x: x["W"].round(2),
        B_pct  = lambda x: x["B_pct"].round(1),
        W_pct  = lambda x: x["W_pct"].round(1),
    )
    .rename(columns={
        "year":         "Year",
        "T":            "Total Gap (pts)",
...
pub_table(
    ...
    small=True,
    note=(
        r"Black--White gap only; ACT Composite scale (1--36). "
        r"Total Gap ($T$) = Between ($B$) + Within ($W$) by construction. "
        r"``Between'' = share attributable to Black and White students attending "
        r"districts with different average ACT scores; ``Within'' = share present "
        r"within the same district. $N$ Dist.\ = number of districts with "
        r"non-suppressed data for both racial groups. "
        r"COVID year 2020--21 excluded; 2019--20 had no state testing."
    ),
)
```
...
```{python}
#| label: tbl-act-mmsd
#| tbl-cap: "ACT Composite Score for Black Students: MMSD and Peer Districts (Grade 11, 2015--16 to 2022--23)"
#| tbl-pos: H
#| output: asis
...
_peers_bw = (
    ...
    .groupby(["district_label", "tier"])
    .agg(
        mean_score = ("avg_score", "mean"),
        n_years    = ("avg_score", "count"),
    )
    .reset_index()
    .rename(columns={
        "district_label": "District",
        "tier":           "Tier",
        "mean_score":     "Mean ACT Composite",
        "n_years":        "Years observed",
    })
    .sort_values(["Tier", "Mean ACT Composite"], ascending=[True, False])
    .round({"Mean ACT Composite": 2})
)
...
pub_table(
    ...
    floatfmt=".2f",
    note=(
        r"Mean ACT Composite score (1--36 scale) for Black students, averaged across all "
        r"available primary years (2015--16 to 2022--23, excluding 2019--20 and 2020--21). "
        r"Tier 0 = MMSD (focal district); Tier 1 = large urban districts "
        r"(Milwaukee, Racine, Kenosha, Green Bay); "
        r"Tier 2 = mid-size districts with substantial minority enrollment; "
        r"Tier 3 = Madison-region suburban districts. "
        r"``Years observed'' = number of school years with non-suppressed Black ACT data."
    ),
)
```
...
## School-Level White and Minority Proficiency (Statewide)

At the school level, DPI suppresses race-specific proficiency for small
subgroups, so only a subset of buildings have both White and Black (or
White and Hispanic) proficiency rates observed. @fig-school-scatter
summarizes Wisconsin public schools with non-suppressed data: Panel A
places White proficiency on the horizontal axis and Black proficiency on
the vertical axis; Panel B uses Hispanic proficiency on the vertical axis.
Madison Metropolitan, Milwaukee, and all other districts are shown in
distinct colors; the figure note reports Pearson correlations between the
axes for the overall sample and for each of those three district groups.

![School-Level White vs. Black and White vs. Hispanic ELA Proficiency](../output/figures/fig09_school_scatter_bw_hw.png){#fig-school-scatter width=100%}

Two patterns stand out. First, **Black student proficiency in MMSD is only
weakly associated with the White proficiency level of the same school**:
among MMSD schools with both rates observed, the correlation is modest
(approximately **0.2**), whereas statewide and in the “other districts”
remainder the White–Black correlation is substantially higher (roughly
**0.45–0.55** in those subsamples, and higher still in Milwaukee). In practical terms, attending an MMSD
school with stronger White proficiency is *not* a strong signal of stronger
Black proficiency in that building—consistent with the view that the
within-MMSD racial gap is not primarily driven by a simple “school quality”
ordering on which Black and White students move in tandem. Second, **the
White–Hispanic correlation is stronger and more consistent across
subsamples**: overall and within MMSD, Milwaukee, and other districts,
correlations are typically in the **0.6–0.8** range (see figure note for
exact values). Hispanic proficiency co-moves more tightly with White
proficiency at the school level than Black proficiency does, especially in
MMSD. These school-level scatters complement the within-MMSD boxplots and
decomposition below: they describe *alignment* of racial group means within
schools, not causal mechanisms.
...
for race in PRIMARY_RACES_A:
    ...
    records.append({
        'Race Group':        race,
        'Dist. cells':       len(d_r),
        'Dist. supp. (\\%)': round(d_r['pct_proficient'].isna().mean()*100, 1),
        'School cells':      len(s_r),
...
pub_table(
    ...
    floatfmt=".1f",
    note=(
        r"A cell is suppressed when the tested count for that race group is $<10$ "
        r"(DPI reporting threshold). "
        r"``Cells'' = total district $\times$ race $\times$ year $\times$ subject $\times$ grade observations "
        r"in the primary years (2015--16 through 2022--23, excluding 2019--20 and 2020--21). "
        r"Suppression rates reflect missingness in the \textit{pct\_proficient} column."
    ),
)
```
...
```{python}
#| label: tbl-supp-year
#| tbl-cap: "District-Level Suppression Rates by Race and Year (Forward Exam)"
#| tbl-pos: H
#| output: asis
...
supp_yr.columns.name = None
pub_table(
    supp_yr,
    colalign=("left",) + ("right",) * (len(supp_yr.columns) - 1),
    floatfmt=".1f",
    small=True,
    note=(
        r"Entries are the percentage of district $\times$ year $\times$ subject $\times$ grade "
        r"cells with suppressed proficiency data in that year. "
        r"A cell is suppressed when the tested count is $<10$. "
        r"Stable rates across years confirm that time-series findings are not driven "
        r"by year-specific changes in tested population coverage."
    ),
)
```
...
agg_d = (
    ...
    .assign(gap_pair=lambda x: x['gap_pair'].map({'BW': 'Black--White', 'HW': 'Hispanic--White'}))
    .rename(columns={
        'year':        'Year',
        'subject':     'Subject',
        'gap_pair':    'Comparison',
...
pub_table(
    ...
    resizebox=True,
    note=(
        r"T = total enrollment-weighted statewide gap (White minus minority proficiency rate, pp). "
        r"T = B + W by construction. B = between-district component; W = within-district component. "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"``Avg.\ $N$'' = average number of districts with valid data for both racial groups, "
        r"across grades 3--8. Values are unweighted averages across grades."
    ),
)
```
...
#| tbl-cap: "MMSD School-Level Black--White Decomposition by Subject (Primary Years)"
#| tbl-pos: H
#| output: asis
...
bw_subj = (
    ...
    .rename(columns={
        ...
        "year":          "Year",
        "T":             "Total Gap (pp)",
        "B_pct":         r"\% Between Schools",
        "W_pct":         r"\% Within Schools",
        "n_schools":     "$N$ Schools",
    })
    .round(1)
)
pub_table(
    ...
    small=True,
    note=(
        r"School-level within-between decomposition of the MMSD Black--White proficiency gap. "
        r"T = total gap (White minus Black, pp). T = Between + Within by construction. "
        r"``ELA+Math (pooled)'' averages ELA and Mathematics scores into a single "
        r"school $\times$ race $\times$ year cell. "
        r"$N$ Schools = number of MMSD schools with non-suppressed data for both "
        r"Black and White students in that year."
    ),
)
```
...
#| tbl-cap: "Proficiency Rates and Racial Gaps With and Without the COVID Year (All Subjects/Grades)"
#| tbl-pos: H
#| output: asis
...
state_c = (
    ...
    .apply(
        lambda g: pd.Series({'pct_prof': weighted_mean_c(g['pct_proficient'], g['n_tested'])}),
        include_groups=False,
    )
    .reset_index()
)
pivot_c = state_c.pivot(index='year', columns='race', values='pct_prof').reset_index()
pivot_c['B--W Gap'] = pivot_c['White'] - pivot_c['Black']
pivot_c['H--W Gap'] = pivot_c['White'] - pivot_c['Hispanic']
pivot_c['Primary sample'] = pivot_c['year'].isin(
    ['2015-16','2016-17','2017-18','2018-19','2021-22','2022-23']
).map({True: 'Yes', False: 'No (COVID)'})
covid_tbl = pivot_c[['year','White','Black','Hispanic','B--W Gap','H--W Gap','Primary sample']].round(1)
covid_tbl.columns = ['Year', 'White (\\%)', 'Black (\\%)', 'Hispanic (\\%)',
                     'B--W Gap (pp)', 'H--W Gap (pp)', 'Primary sample']
pub_table(
    covid_tbl,
    colalign=("left", "right", "right", "right", "right", "right", "left"),
    floatfmt=".1f",
    small=True,
    note=(
        r"Proficiency rates and gaps are enrollment-weighted averages across all "
        r"subjects (ELA and Mathematics) and grades 3--8. "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"2019--20 is excluded (no assessment administered). "
        r"2020--21 is the COVID disruption year: lower and non-representative participation."
    ),
)
```
...
```{python}
#| label: tbl-wkce-gaps
#| tbl-cap: "Statewide WKCE Scale-Score Gaps by Year (Grades 3--8, Enrollment-Weighted)"
#| tbl-pos: H
#| output: asis

# Pivot to wide: rows = year (6), cols = B--W/H--W × Reading/Math gap values
_bw = wkce_gaps[wkce_gaps["gap_pair"] == "BW"].pivot(
    index="year", columns="subject", values="gap"
).rename(columns={"Reading": "B--W Reading", "Math": "B--W Math"})

_hw = wkce_gaps[wkce_gaps["gap_pair"] == "HW"].pivot(
    index="year", columns="subject", values="gap"
).rename(columns={"Reading": "H--W Reading", "Math": "H--W Math"})
...
wkce_wide = (
    ...
    .reset_index()
    .rename(columns={"year": "Year"})
)[["Year", "B--W Reading", "B--W Math", "H--W Reading", "H--W Math"]]
...
pub_table(
    ...
    floatfmt=".1f",
    note=(
        r"Entries are district-enrollment-weighted state-average scale-score gaps "
        r"(White mean minus minority mean, WKCE scale-score points), "
        r"aggregated across grades 3--8. "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"White statewide mean $\approx 504$ (Reading) and $\approx 512$ (Math). "
        r"Years 2009--10 through 2013--14; 2007--08 excluded due to data issues."
    ),
)
```
...
tbl2 = (
    ...
    .round(1)
    .sort_values(["Comparison", "Subject", "Year"])
)
pub_table(
    tbl2,
    colalign=("left", "left", "left", "right", "right", "right", "right", "right", "right"),
    ...
    resizebox=True,
    note=(
        r"T = total district-enrollment-weighted statewide scale-score gap (White minus minority). "
        r"T = B + W by construction. B--W = Black--White; H--W = Hispanic--White. "
        r"Grades 3--8 aggregated (enrollment-weighted within district). "
        r"$N$ Dist.\ = districts with valid data for both racial groups."
    ),
)
```
...
#| tbl-cap: "WKCE-Era Mean Reading Scale Scores: MMSD and Peer Districts (Black and Hispanic, 2009--10 to 2013--14)"
#| tbl-pos: H
#| output: asis
...
tbl3 = (
    ...
    .rename(columns={
        ...
        "tier":           "Tier",
        "race":           "Race",
        "mean_ss":        "Mean SS (Reading)",
    })
    .round(1)
    .sort_values(["Race", "Mean SS (Reading)"], ascending=[True, False])
)
pub_table(
    tbl3,
    ...
    small=True,
    note=(
        r"Mean WKCE Reading scale score, averaged across all available years "
        r"(2009--10 through 2013--14). "
        r"Tier 0 = MMSD; Tier 1 = large urban districts; "
        r"Tier 2 = mid-size urban; Tier 3 = Madison-region suburban. "
        r"WKCE scale scores have statewide White mean $\approx 504$."
    ),
)
```
...
```{python}
#| label: tbl-wkce-gr10
#| tbl-cap: "WKCE Grade-10 State-Level Scale-Score Gaps by Year (Reading and Mathematics, 2009--10 to 2012--13)"
#| tbl-pos: H
#| output: asis

wkce_g10 = pd.read_csv(ROOT / "output/tables/wkce_grade10_gaps.csv")

# Pivot to wide: rows = year (5), cols = B--W/H--W × Reading/Math
_bw10 = wkce_g10[wkce_g10["gap_pair"] == "BW"].pivot(
    index="year", columns="subject", values="gap"
).rename(columns={"Reading": "B--W Reading", "Math": "B--W Math"})

_hw10 = wkce_g10[wkce_g10["gap_pair"] == "HW"].pivot(
    index="year", columns="subject", values="gap"
).rename(columns={"Reading": "H--W Reading", "Math": "H--W Math"})
...
pub_table(
    ...
    floatfmt=".1f",
    note=(
        r"WKCE scale-score gaps (White mean minus minority mean, scale-score points) at grade 10. "
        r"Grade-10 scale scores are on a higher part of the developmental scale than grades 3--8; "
        r"absolute gaps are not directly comparable across grade bands. "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"2013--14 grade-10 data are incomplete and excluded."
    ),
)
```
...
```{python}
#| label: tbl-crossera
#| tbl-cap: "Cross-Era Comparison: Key Racial Achievement Gap Indicators (WKCE 2009--14 vs. Forward Exam 2015--23)"
#| tbl-pos: H
#| output: asis

rows = [
    ("WKCE (2009--14)",    "Reading", "Black--White",    "44--48 pts",       "55--58\\%", "42--45\\%"),
    ("WKCE (2009--14)",    "Math",    "Black--White",    "49--55 pts",       "57--60\\%", "40--43\\%"),
    ("WKCE (2009--14)",    "Reading", "Hispanic--White", "34--37 pts",       "37--40\\%", "60--63\\%"),
    ("Forward (2015--23)", "ELA",     "Black--White",    "$\\sim$38--40 pp", "$\\sim$47\\%", "$\\sim$53\\%"),
    ("Forward (2015--23)", "Math",    "Black--White",    "$\\sim$42--44 pp", "$\\sim$45\\%", "$\\sim$55\\%"),
    ("Forward (2015--23)", "ELA",     "Hispanic--White", "$\\sim$24 pp",     "$\\sim$40\\%", "$\\sim$60\\%"),
]
cross_df = pd.DataFrame(rows, columns=[
    "Era", "Subject", "Comparison", "Gap Magnitude", r"\% Between", r"\% Within"
])
pub_table(
    cross_df,
    ...
    resizebox=True,
    note=(
        r"WKCE Gap Magnitude is in scale-score points (WKCE developmental scale); "
        r"Forward Exam Gap Magnitude is in proficiency-rate percentage points (pp). "
        r"The two metrics are not directly comparable across eras. "
        r"Between/Within shares are district-level decomposition estimates. "
        r"B--W = Black--White; H--W = Hispanic--White. "
        r"MMSD vs.\ peer districts: in the WKCE era, MMSD Black students perform "
        r"comparably to Milwaukee Black students (Reading $\approx$453 vs.\ 451 pts); "
        r"in the Forward era, MMSD Black students perform \textit{below} Milwaukee "
        r"Black students (8--10\% vs.\ 16--22\% proficient in ELA). "
        r"MMSD Hispanic students perform slightly above peers in the WKCE era "
        r"and approximately at peers in the Forward era."
    ),
)
```
...
## E. Economic Disadvantage vs. Non-Disadvantage (School-Level) {.unnumbered}

The main text uses race/ethnicity cuts from the Forward Exam. As a
supplementary exercise, we replicate the **school-level scatter** design as
closely as the public data allow using DPI’s **Economic Status** breakdown:
"Not Econ Disadv" (not economically disadvantaged) and "Econ Disadv"
(economically disadvantaged). This is **not** a substitute for the
Black–White or Hispanic–White analysis: economic status is correlated with
race, but it is a distinct construct, and we do not observe race × poverty
cells at suppressed-safe school-level sample sizes here.

@fig-econ-scatter plots, for each school with both rates non-suppressed,
non-ED proficiency on the horizontal axis and ED proficiency on the
vertical axis (ELA only, pooled across grades 3–8 and primary years, same
construction as @fig-school-scatter). Colors distinguish MMSD, Milwaukee,
and other districts. Pearson correlations by subsample are printed in the
figure note.

![School-Level Non-ED vs. ED ELA Proficiency (DPI Economic Status)](../output/figures/fig_A10_econ_school_scatter.png){#fig-econ-scatter width=85%}

**Pattern.** The association between non-ED and ED proficiency at the same
school is **strong and positive statewide** (correlation approximately **0.74**
in the overall sample), and similarly high within MMSD, Milwaukee, and other
districts—substantially stronger than the **White–Black** school-level
correlation in MMSD in @fig-school-scatter. In other words,
schools that are better for non-economically disadvantaged students also
tend to be better for economically disadvantaged students in a way that
Black and White proficiency do not move together within MMSD buildings.
This appendix is descriptive only; it does not identify causal effects of
economic status or school assignment.
